...
- Avoid unnecessary object creation (autoboxing, duplicate `String` instances, etc.)
- Control how and when instances are created
- Reuse shared instances where appropriate:
  - preferably immutable objects
  - mutable objects when sharing is safe and well-defined
- When designing classes:
  - Design them as instance-controlled where appropriate (singletons, enum-based classes)
  - When instances are meaningless, design them as final utility classes to prevent instantiation
...
Considerations about **when** and **why**:
