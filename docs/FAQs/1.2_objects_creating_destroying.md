...
Advantages:
- [provide expressive naming](#named-static-factory-methods-compare-with-a-constructor)
- control how and when instances are created:
  - [help to avoid creating unnecessary duplicate objects](1.3_objects_creating_destroying.md#unnecessary-objects-constructors-vs-static-factories)
  - [can be used by instance-controlled classes](#instance-controlled-classes)
- decouple API from implementation
  - [support **interface‑based design**, where the API returns an interface type and hides concrete implementations](#choosing-the-class-of-the-returned-object-when-it-gets-created)
  - [select an implementation based on input parameters, allowing the factory to return the most appropriate implementation (e.g., for performance or algorithmic efficiency)](#how-to-vary-the-class-of-the-created-object-from-call-to-call-as-a-function-of-the-input-parameters-motivation)
  - [support **service‑provider frameworks**, where the returned implementation may not exist at the time the API is written](#service-provider-frameworks)
...
When applying the Builder pattern to class hierarchies, inheritance must be used with care. 


Concrete, instantiable value classes should not form inheritance hierarchies, because they are not safely extensible; 
composition should be preferred instead. 


Inheritance becomes safe in this context only when the base class is abstract and non‑instantiable. 
Only then can a class hierarchy be designed for extension, making the Builder pattern appropriate and safe for hierarchies.
