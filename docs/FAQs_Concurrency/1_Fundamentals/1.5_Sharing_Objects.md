### Sharing a mutable variable — best approach?
<details><summary>Show answer</summary>

Avoid sharing the same mutable object where you can. Ways, in order of preference:

- **Confine it to one thread** — if only one thread ever touches the data, there is no sharing and nothing
  to synchronize.
- **Give each thread its own copy** — a defensive copy hands out state disconnected from yours, so one
  thread's changes can't reach another's. Works when:
  - the object is cheap to copy and 
  - threads don't need to share the *result*, only to avoid colliding on the input.
- **Share it immutable** — data that never changes after construction is safe for any number of threads to
  read at once.

If none of these fit and you are forced to share the same mutable object live, you cannot remove the
problem — you have to manage it, and [there are two things you must ensure](#what-does-safe-sharing-require).

</details>

### What does safe sharing require?
<details><summary>Show answer</summary>

- **Atomicity** — a read or write completes in one step, so no thread sees it half-done.
- **Visibility** — a write by one thread actually reaches the threads that read it.

Atomicity alone fails: the value is whole, but a reader may keep seeing an old copy forever.
Visibility alone fails: the update reaches readers, but a read-modify-write (like `i++`) still lets two
threads collide. Sharing safely means covering both.

</details>

### Safe sharing vs synchronization?
<details><summary>Show answer</summary>

Different levels — one is the goal, the other is one way to reach it:

- **Safe sharing** — the goal: many threads use the same mutable data without corruption.
- **Synchronization** — one mechanism that delivers both, for any block of code.

Synchronization is not the only way to share safely — it is the one that covers both guarantees at once.
Others cover part or sidestep the need entirely:
- `volatile` gives visibility only,
- an atomic type gives both for a single variable.

Reach for synchronization when you must share live mutable state and need both — visibility and atomicity — at once.

</details>

### Sharing a mutable object without ensuring atomicity or visibility?
<details><summary>Show answer</summary>

Possible when the object stops changing before it is shared. 
One thread builds it by modifying it freely, then stops, and shares the reference. 
If it is never changed again, other threads read it and you need neither atomicity nor visibility on those reads.


Both requirements fall away for the same reason — there are no writes after publication. 
Nothing to make atomic (no read-modify-write is happening), and nothing to miss (the value never changes again). 
You cover visibility once, on the hand-off, so readers see a fully-built object; after that every read is free.

The object was mutable while built, but from publication on it never changes, so readers treat it as immutable. 
Such an object is called **effectively immutable**.

</details>

### Effectively immutable — what does it mean?
<details><summary>Show answer</summary>

An object that is not immutable by construction, but whose state is never changed after a point — so it
behaves as immutable and can be shared for reads without synchronization.

Two ways the "never changed" holds:

- **Nobody writes it after publication.** The object is shared, but by agreement no thread modifies it once handed over. 
  Unchanged by promise.
- **Nobody else can reach it.** The owner keeps exclusive control — the state is never exposed or published, 
  so no outside code holds a reference to change it. Unchanged because it is unreachable — see 
  [exclusive control of the object graph](../../FAQs/04_Classes_&_Interfaces/4.2_classes_and_interfaces.md#what-is-required-to-keep-mutable-components-effectively-immutable)

Same result — state that never changes — reached two ways: a promise in the first, no access in the second.

</details>

### Passing a built object between threads safely?
<details><summary>Show answer</summary>

Hand the reference over in a way that guarantees the receiving thread sees the object fully built — all its
fields written, not half-constructed. This correct hand-off is called **safe publication**.

How you actually do it: [safe publication needs the right kind of hand-off](#how-do-you-safely-publish-a-reference).
When it goes wrong: [unsafe publication](#how-can-publication-go-wrong).

</details>

### How do you safely publish a reference?
<details><summary>Show answer</summary>

Any hand-off that forces the writes-then-reference to become visible together — each way carries that
guarantee, so you don't memorize a list, you recognize the mechanism:

- **Static field set during class init** — the JVM guarantees class initialization is seen by all threads.
- **`final` field** — the memory model guarantees a `final` field is visible once the constructor finishes.
- **`volatile` field** — a write is visible to any thread that later reads it.
- **Field guarded by a lock** — the reader takes the same lock, so it sees the writer's earlier writes.
- **A concurrent collection** — putting the reference in guarantees a safe hand-off to whoever takes it out.

The through-line: every one draws a happens-before line at the moment of sharing, so the writes that
built the object land before the reader touches it.

</details>

### How can publication go wrong?
<details><summary>Show answer</summary>

Making the reference visible is not enough on its own. The writes that built the object and the write of the
reference are separate — another thread can see the reference before it sees those field writes, and read an
object that is only half-constructed.

The reference arrives, but the fields it points to are still being written. So the reader gets a valid-looking
pointer to a broken object — no error, just wrong data. This is **unsafe publication**, and it's what
[safe publication](#passing-a-built-object-between-threads-safely) exists to prevent.

</details>

### Safe publication vs effectively immutable?
<details><summary>Show answer</summary>

They answer different questions, and you usually need both:

- **Effectively immutable** — a property of the *object*: its state never changes after a point, 
  so  readers need no synchronization to read it.
- **Safe publication** — a property of the *hand-off*: 
  the reference is passed so the receiving thread sees the object fully built, not half-made.

They combine, not compete. Effectively immutable makes the object safe to read; 
safe publication makes sure the reader gets the finished object in the first place. 
An effectively immutable object published *unsafely* is still unsafe — 
a thread can see the reference before the object's writes, and read a half-built object. 
Immutable-after-a-point removes the need to lock later reads; 
safe publication is what makes that first hand-off correct.

</details>


### Why is an immutable object thread-safe for free?
<details><summary>Show answer</summary>

Its state can't change, so every problem synchronization solves simply cannot occur. No thread can catch it
half-updated (no inconsistent middle), no update can be lost (nothing does read-modify-write on it), and there is
no stale write to publish (the value never changes after construction). So it needs no locks, no `volatile`, no
synchronization at all — many threads read it at once, safely.

```java
public final class Deposit {          // final class, all fields final
  private final double amount;
  private final Account payee;
  // set once in the constructor, never again
}
```

This is the opposite end from a fully synchronized object: that design *worked* to reach safety by locking every
method; immutability gets the same safety by removing the thing locks protect — mutable state.

Handle: no state change means no race — an immutable object is safe and live by construction, with no locking.

</details>

### Is final enough to share an object without locks?
<details><summary>Show answer</summary>

Yes for `final` fields, and this is a specific JMM guarantee — not just "the reference can't be reassigned."

When a constructor finishes, any thread that then gets the object is guaranteed to see its `final` fields fully
initialized, **without** synchronization. This is **safe publication**: `final` fields are correctly published for
free. Non-final fields get no such promise — another thread can see a non-final field still at its default (`0`,
`null`) even after the constructor returned, unless you synchronize.

```java
class C {
  final int a;          // guaranteed visible after construction
  int b;                // NOT guaranteed — another thread may see 0
  C() { a = 1; b = 2; }
}
```

The one catch: the guarantee holds only if the object does **not** leak `this` during construction (e.g. registers
itself in a shared collection before the constructor finishes). Leak `this` early and even final fields can be
seen half-built.

Handle: `final` fields are safely published — visible fully-built to other threads with no lock — as long as
`this` doesn't escape the constructor; non-final fields have no such guarantee.

</details>

### Does a final field make an object immutable?
<details><summary>Show answer</summary>

No. `final` freezes the *reference*, not the object it points to. A `final` field holding a mutable object is
still mutable through that field — and not thread-safe.

```java
final var numbers = new LinkedList<Integer>();
numbers.add(1);        // fine — the list is still mutable
// numbers = otherList; // this is what final forbids — reassigning the reference
```

The reference can never point elsewhere, but the list it points to can be added to, removed from, cleared — by any
thread, with no safety. This is **shallow immutability**: the top reference is fixed, the target is not. An object
is only truly immutable (and only thread-safe for free) if its fields are final *and* point at immutable things.

Handle: `final` locks the reference, not the target — a `final` field on a mutable object is shallow
immutability, still mutable and still not thread-safe.

</details>

### How do you change an immutable object safely?
<details><summary>Show answer</summary>

You don't change it — you return a **new** object with the changed field, leaving the original untouched.

```java
public Deposit roll() {                        // move the deposit to the next day
  return new Deposit(amount, date.plusDays(1), payee);
}
public Deposit amend(double newAmount) {       // change the amount
  return new Deposit(newAmount, date, payee);
}
```

`roll()` and `amend()` don't mutate `this` — they build a fresh `Deposit` sharing the unchanged fields. In a
concurrent program this is why immutability composes: no thread ever observes a *change* to a shared object,
because the object never changes. A "change" is one thread swapping its own reference to point at the new object;
every other thread still holds the old one, fully intact.

Handle: change an immutable object by returning a new one, not mutating it — no thread sees a change because the
object never changes, only which instance you hold.

</details>

### Why isn't a builder thread-safe — and why is that fine?
<details><summary>Show answer</summary>

A builder is deliberately mutable — it collects fields one call at a time — so it is *not* thread-safe, and it's
not meant to be. The design confines the mutable phase to **one thread**: that thread accumulates all the state,
then calls `build()` to produce an immutable object, which can then be shared freely.

```java
Deposit d = new DepositBuilder()
  .amount(100).date(today).payee(acct)   // one thread mutating the builder
  .build();                              // produces an immutable Deposit — now safe to share
```

Only the finished immutable result crosses thread boundaries; the mutable builder never does. This is **thread
confinement** — keep mutable state on a single thread and share only the frozen result. Wanting to share a builder
across threads is a sign the design is wrong.

Handle: a builder is mutable and single-thread by design — confine it to one thread, share only the immutable
object it builds.

</details>

### [scope check] Static factory over constructor — why?
<details><summary>Show answer</summary>

For an immutable class with many fields, factory methods read better than raw constructors and can carry
convenience overloads. The constructor is made `private`, so the static factory is the only way in:

```java
private Deposit(double amount, LocalDate date, Account payee) { ... }

public static Deposit of(double amount, LocalDate date, Account payee) {
  return new Deposit(amount, date, payee);
}
public static Deposit of(double amount, Account payee) {   // convenience: today's date
  return new Deposit(amount, LocalDate.now(), payee);
}
```

Named factories can also carry meaning a constructor can't (`of`, `copyOf`), return a cached instance instead of a
new one, and vary the returned type — none of which a constructor allows.

Handle: a private constructor plus static factories gives named, overloadable, cacheable creation — the standard
entry point for an immutable class.

</details>

### [scope check] Builder as a functional interface — should you?
<details><summary>Show answer</summary>

The `Builder<T>` interface has one abstract method (`build()`), so it is technically a **SAM** type — a single
abstract method interface — and could be the target of a lambda. But you shouldn't treat it as one.

```java
public interface Builder<T> { T build(); }
```

A functional interface represents a *function or callback*. A builder's job is to *gather state* and produce an
object — a different purpose that happens to share the one-method shape. So it is left without the
`@FunctionalInterface` annotation on purpose: the annotation signals "this is meant to be used as a function,"
which a builder is not.

Handle: one abstract method makes an interface usable as a lambda, but not meant for one — a builder is a
state-gatherer, so no `@FunctionalInterface`. ("Just because you can doesn't mean you should.")

</details>

