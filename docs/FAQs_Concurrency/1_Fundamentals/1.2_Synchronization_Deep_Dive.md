...
**Lock works, but not optimal — a lighter tool is preferred:**

- **Read-modify-write on shared data** — `i++`, or check-then-act. 
  Two threads interleave the steps and one update is lost. 
  An atomic type like `AtomicInteger` covers it without blocking. *(atomicity)*
- **A write that never reaches another thread** — one thread writes, another keeps reading a stale copy forever.
  `volatile` publishes the write without mutual exclusion. *(visibility)*

**Lock is the only solution:**

- **A multi-step change across several fields** — the object must move from one good state to another with no
  thread seeing the middle. More than one variable, so no atomic type reaches it — only a lock makes the whole
  change indivisible. *(atomicity across a block)*
...
- **Right tool** — for one wide read/write or one read-modify-write, 
  [a lighter tool does the job](#which-problems-need-synchronization);
  a synchronized block there is too heavy.
- **Correctness** — never call code you don't control from inside the lock, or it can
  [deadlock or corrupt state](../5_Liveness_Performance_Testing/5.1_Liveness_Problems.md#synchronized-block--how-to-avoid-liveness-problems).
- **Performance** —
    - every other thread [waits while you hold the lock](../5_Liveness_Performance_Testing/5.2_Performance_&_Scalability.md#what-does-synchronization-cost-in-performance),
    - so [keep the guarded part small](../5_Liveness_Performance_Testing/5.2_Performance_&_Scalability.md#reducing-synchronizations-performance-cost).
