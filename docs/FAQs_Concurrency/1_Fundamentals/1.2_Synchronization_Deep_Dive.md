...
- **Correctness** — never call code you don't control from inside the lock, or it can
  [deadlock or corrupt state](../5_Liveness_Performance_Testing/4.1_Liveness_Problems.md#synchronized-block--how-to-avoid-liveness-problems).
- **Performance** —
    - every other thread [waits while you hold the lock](../5_Liveness_Performance_Testing/4.2_Performance_&_Scalability.md#what-does-synchronization-cost-in-performance),
    - so [keep the guarded part small](../5_Liveness_Performance_Testing/4.2_Performance_&_Scalability.md#reducing-synchronizations-performance-cost).
